...
function ENT:GetTracer()
	...
	local td = {}
		td.start = self:GetTurretOwner():EyePos() + self:GetTurretOwner():EyeAngles():Forward() * 256
		td.endpos = td.start + self:GetTurretOwner():EyeAngles():Forward() * 30000
		td.filter = { self.Entity, self:GetTurretOwner() }
...
function ENT:TurningTurret(ct)
	...
	if (self:GetReady() == true) and self:GetTurretOwner():IsValid() and self:GetTurretOwner():InVehicle() and (ct > self:GetReloadTime()) then
...
function ENT:Aiming(ct)
	...
	if (ct > (self.LastShoot + self.Cooldown)) then
		if self:GetTurretOwner():KeyDown(GetConVarNumber("tnt_turret_fire")) then
